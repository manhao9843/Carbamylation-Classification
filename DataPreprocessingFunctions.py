...
def who_is_nearby(pdb_file, resid, chain, d):
    ...
    # NZ position of the lysine as the origin
    M = bb.Molecule()
    ...
    for line in read_file:
        ...
        if distance <= 12:
            ...
            # mark the resid identified
            skip_resid = line[22:26].strip()
            current_resid = int(line[22:26].strip())
            
            # include if it is not the target lysine
...
class loader(object):
    def __init__(self, df, d):
        """
        df: df_train / df_test
        d: cut-off distance defined by the user
...
class Encoder(object):
    def __init__(self, L):
        """
        L: an instance of loader object
        ...
        # map amino acids to integers, apply position encoding to coordinates
        for idx, data in L.dict.items():
            sample = list()
            # put lys at the beginning 
